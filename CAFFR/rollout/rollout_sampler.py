...
class R_ITER():
    ...
    N_workers : int
        Expected number of workers to have, this is needed here to make all the trajector
    ...
    """
    def __init__(self, env, H, N_workers, gamma, K, lookahead, 
            ...
            self.H_args = H_args
            self.chunks = ceil(self.actions_c**lookahead / self.cpus) # Size of chuncks
            self.j = lookahead - 1
            self.max_index = []
            self.t_pos = []
            ...
            self.all_trajector = [] # Storing all trajector. It's better to complextity to memory than
        ...
        while not self.is_constructed:
            ...
            for _ in range(self.chunks):
                ...
                if self.is_constructed:
                    ...
                    chunck_trajector += [branch]
            self.all_trajector += [chunck_trajector]
        return True
    @property
    ...
    def __next__(self):
        if self.i == self.cpus:
            ...
            To_H = {
                    ...
                    'H':self.H_ref,
                    'trajector':self.all_trajector[self.i],
                    'gamma':self.gamma,
                    'K':self.K,
...
def sample_trayectory(args):
    ...
    trajector. At the end returns the best action with the expected
    cost associated.
    ...
    'gamma' : Discount factor for the cost function.
    ...
    LIMIT_OF_STEPS = 10**3
    # loading variables
    expected_args = ['env', 'trajector', 'H', 'gamma', 'K', 'N_SAMPLES','min_obj']
    ...
    trajector = args['trajector']
    l_trajector = len(trajector)
    assert l_trajector > 0, "This function should not sample empty trajector. Please check your inputs."
    l_trayectory = len(trajector[0])
    ...
    for t in range(l_trajector):
        trayectory = trajector[t]
        total_cost = 0.0
        for _ in range(N_SAMPLES):
            GAMMA = gamma # Start the path with Alpha
            sample_cost = 0.0
            action_cost = 0.0
