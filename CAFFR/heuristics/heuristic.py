...
def Heuristic(args):
    ...
    expected_args = ['env','observation','vision','mode']
    ...
    if mode==1:
        up_left_burned=Count_Burned_Trees(env,up_left_zone)
    elif mode==2:
        up_left_burned=Calculate_Fire_Coefficient(env,up_left_zone) 
...
def get_neighborhood(grid,pos,vision):
    pos_row=pos[0]
    pos_col=pos[1]    
    neighborhood=grid[pos_row :pos_row+1+2*vision,pos_col:pos_col+1+2*vision] #RWH
    return neighborhood

def ExpandGrid(grid,vision):        
        size = grid.shape        
        PadGrid = np.zeros((size[0],size[1]), dtype=np.int8)        
